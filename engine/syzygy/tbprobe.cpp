/*
  This code is copied from Stockfish and adjusted to work in chessplusplus.
*/
...
using namespace engine::Tablebases;

int engine::Tablebases::MaxCardinality;
int engine::Tablebases::ProbeLimit;
int engine::Tablebases::ProbeDepth;
bool engine::Tablebases::Use50Rule;
...
int MapPawns[SQUARE_NUM];
int MapB1H1H7[SQUARE_NUM];
int MapA1D1D4[SQUARE_NUM];
int MapKK[10][SQUARE_NUM]; // [MapA1D1D4][SQUARE_NB]

int Binomial[6][SQUARE_NUM];    // [k][n] k elements from a set of n elements
int LeadPawnIdx[6][SQUARE_NUM]; // [leadPawnsCnt][SQUARE_NB]
int LeadPawnsSize[6][4];       // [leadPawnsCnt][FILE_A..FILE_D]

Piece stockfishPieceToMyPiece(int stockfishPiece) {
    std::array<Piece, 15> pieces = {
        NO_PIECE, W_PAWN, W_KNIGHT, W_BISHOP, W_ROOK, W_QUEEN, W_KING, NO_PIECE,
        NO_PIECE, B_PAWN, B_KNIGHT, B_BISHOP, B_ROOK, B_QUEEN, B_KING};
    return pieces[stockfishPiece];
}

// Comparison function to sort leading pawns in ascending MapPawns[] order
bool pawns_comp(Square i, Square j) { return MapPawns[i] < MapPawns[j]; }
int off_A1H8(Square sq) { return int(rank(sq)) - file(sq); }

/* constexpr Value WDL_to_value[] = { */
/*    -VALUE_MATE + MAX_PLIES + 1, */
/*     VALUE_DRAW - 2, */
/*     VALUE_DRAW, */
/*     VALUE_DRAW + 2, */
/*     VALUE_MATE - MAX_PLIES - 1 */
...
TBTable<WDL>::TBTable(const std::string& code) : TBTable() {
    ...
    const int numWhite = std::count(codeWhite.begin(), codeWhite.end(), 'P');
    const int numBlack = std::count(codeBlack.begin(), codeBlack.end(), 'P');

    key = HashKey(code, WHITE).get_key();
    pieceCount = code.length() - (code.find('v') != std::string::npos ? 1 : 0);
    hasPawns = (numWhite + numBlack) > 0;

    hasUniquePieces = false;
    for (char c : {'P', 'N', 'B', 'R', 'Q'})
        if (std::count(codeWhite.begin(), codeWhite.end(), c) == 1
                || std::count(codeBlack.begin(), codeBlack.end(), c) == 1)
            hasUniquePieces = true;

    // Set the leading color. In case both sides have pawns the leading color
    // is the side with less pawns because this leads to better compression.
    bool c = !numBlack || (numWhite && numBlack >= numWhite);

    pawnCount[0] = c ? numWhite : numBlack;
    pawnCount[1] = c ? numBlack : numWhite;

    key2 = HashKey(code, BLACK).get_key();
}
...
class TBTables {
    ...
    void insert(uint64_t key, TBTable<WDL>* wdl, TBTable<DTZ>* dtz) {
        uint32_t homeBucket = (uint32_t)key & (Size - 1);
        Entry entry{ key, wdl, dtz };

        // Ensure last element is empty to avoid overflow when looking up
        for (uint32_t bucket = homeBucket; bucket < Size + Overflow - 1; ++bucket) {
            uint64_t otherKey = hashTable[bucket].key;
...
public:
    ...
    void add(const std::vector<PieceKind>& pieces);
};
...
void TBTables::add(const std::vector<PieceKind>& pieces) {

    std::string code;

    for (PieceKind pt : pieces)
        code += PieceKindToChar[pt];
...
Ret do_probe_table(const Position& pos, T* entry, WDLScore wdl, ProbeState* result) {
    ...
    bool symmetricBlackToMove = (entry->key == entry->key2 && pos.color());
    ...
    bool blackStronger = (pos.material_hash() != entry->key);

    bool flipColor   = symmetricBlackToMove || blackStronger;
    bool flipSquares = symmetricBlackToMove || blackStronger;
    int stm         = (symmetricBlackToMove || blackStronger) ^ pos.color();
    ...
    if (entry->hasPawns) {
        ...
        Piece pc = piece_flip_color(entry->get(0, 0)->pieces[0], flipColor);

        assert(get_piece_kind(pc) == PAWN);

        leadPawns = b = pos.pieces(get_color(pc), PAWN);
        do
        {
            Square sq = Square(pop_lsb(&b));
            squares[size++] = flipSquares ? flip_vertically(sq) : sq;
        }
        while (b);
        ...
        tbFile = File(std::min(file(squares[0]), FILE_H - file(squares[0])));
    }
    ...
    do {
        Square s = Square(pop_lsb(&b));
        squares[size] = flipSquares ? flip_vertically(s) : s;
        pieces[size++] = piece_flip_color(pos.piece_at(s), flipColor);
    } while (b);
    ...
    if (file(squares[0]) > FILE_D)
        for (int i = 0; i < size; ++i)
            squares[i] = flip_horizontally(squares[i]);
    ...
    if (rank(squares[0]) > RANK_4)
        for (int i = 0; i < size; ++i)
            squares[i] = flip_vertically(squares[i]);
    ...
    if (entry->hasUniquePieces) {
        ...
        // All 3 pieces on the diagonal a1-h8
            ...
            idx = 6 * 63 * 62 + 4 * 28 * 62 + 4 * 7 * 28
                 +  rank(squares[0])         * 7 * 6
                 + (rank(squares[1]) - adjust1)  * 6
                 + (rank(squares[2]) - adjust2);
    } else
...
void set(T& e, uint8_t* data) {
    ...
    for (File f = FILE_A; f <= maxFile; ++f) {
        ...
        for (int k = 0; k < e.pieceCount; ++k, ++data)
            for (int i = 0; i < sides; i++)
                e.get(i, f)->pieces[k] = stockfishPieceToMyPiece(i ? *data >>  4 : *data & 0xF);
...
void* mapped(TBTable<Type>& e, const Position& pos) {
    ...
    for (PieceKind pt = KING; pt >= PAWN; --pt) {
        w += std::string(popcount(pos.pieces(WHITE, pt)), PieceKindToChar[pt]);
        b += std::string(popcount(pos.pieces(BLACK, pt)), PieceKindToChar[pt]);
    }

    fname =  (e.key == pos.material_hash() ? w + 'v' + b : b + 'v' + w)
           + (Type == WDL ? ".rtbw" : ".rtbz");
...
Ret probe_table(const Position& pos, ProbeState* result, WDLScore wdl = WDLDraw) {
    ...
    if (popcount(pos.pieces()) == 2) // KvK
        return Ret(WDLDraw);

    TBTable<Type>* entry = TBTables.get<Type>(pos.material_hash());
...
WDLScore search(Position& pos, ProbeState* result) {
    ...
    Move* start = moveList.data();
    Move* end = generate_moves(pos, pos.color(), start);
    size_t totalCount = end - start, moveCount = 0;

    for (Move* it = start; it != end; ++it)
    {
        Move move = *it;

        if (   !pos.move_is_capture(move)
            && (!CheckZeroingMoves || get_piece_kind(pos.piece_at(from(move))) != PAWN))
            continue;

        moveCount++;

        moveInfo = pos.do_move(move);
        value = -search<false>(pos, result);
        pos.undo_move(move, moveInfo);
...
void Tablebases::init(const std::string& paths) {
    ...
    for (Square s = SQ_A1; s <= SQ_D4; ++s)
        if (off_A1H8(s) < 0 && file(s) <= FILE_D)
            MapA1D1D4[s] = code++;

        else if (!off_A1H8(s) && file(s) <= FILE_D)
        ...
        for (File f = FILE_A; f <= FILE_D; ++f)
        {
            ...
            for (Rank r = RANK_2; r <= RANK_7; ++r)
            {
                ...
                if (leadPawnsCnt == 1)
                {
                    ...
                    MapPawns[flip_horizontally(sq)] = availableSquares--;
                }
    ...
    for (PieceKind p1 = PAWN; p1 < KING; ++p1) {
        TBTables.add({KING, p1, KING});

        for (PieceKind p2 = PAWN; p2 <= p1; ++p2) {
            TBTables.add({KING, p1, p2, KING});
            TBTables.add({KING, p1, KING, p2});

            for (PieceKind p3 = PAWN; p3 < KING; ++p3)
                TBTables.add({KING, p1, p2, KING, p3});

            for (PieceKind p3 = PAWN; p3 <= p2; ++p3) {
                TBTables.add({KING, p1, p2, p3, KING});

                for (PieceKind p4 = PAWN; p4 <= p3; ++p4) {
                    TBTables.add({KING, p1, p2, p3, p4, KING});

                    for (PieceKind p5 = PAWN; p5 <= p4; ++p5)
                        TBTables.add({KING, p1, p2, p3, p4, p5, KING});

                    for (PieceKind p5 = PAWN; p5 < KING; ++p5)
                        TBTables.add({KING, p1, p2, p3, p4, KING, p5});
                }

                for (PieceKind p4 = PAWN; p4 < KING; ++p4) {
            ...
            for (PieceKind p3 = PAWN; p3 <= p1; ++p3)
                for (PieceKind p4 = PAWN; p4 <= (p1 == p3 ? p2 : p3); ++p4)
                    TBTables.add({KING, p1, p2, KING, p3, p4});
        }
    }

    logger_sync_out << "info string Found " << TBTables.size() << " tablebases" << sync_endl;
}
...
int Tablebases::probe_dtz(Position& pos, ProbeState* result) {
    ...
    *result = OK;
    ...
    std::array<Move, MAX_MOVES> moveList;
    Move* start = moveList.begin();
    Move* end = generate_moves(pos, pos.color(), start);
    for (Move* it = start; it != end; ++it)
    {
        Move move = *it;
        bool zeroing = pos.move_is_capture(move) || get_piece_kind(pos.piece_at(from(move))) == PAWN;
...
bool Tablebases::root_probe(Position& pos, std::vector<Move>& rootMoves) {
    ...
    int cnt50 = pos.half_moves();

    // Check whether a position was repeated since the last zeroing move.
    bool rep = pos.is_repeated();
    ...
    for (auto& m : rootMoves)
    {
        ...
        if (dtz == 2 && pos.is_checkmate())
            dtz = 1;

        pos.undo_move(m.pv[0], moveInfo);
        ...
        m.tbScore =  r >= bound ? VALUE_MATE - MAX_PLIES - 1
